...
struct user *adicionarestatico (struct user *ini, char nome[])
{
...
struct user *amizade (struct user *ini)
{
    ...
    while (aux2->proximouser!=NULL && strcmp(nome2,aux2->nome)!=0)
    {
        aux2=aux2->proximouser;
    }
    
    printf("Nome: %s, busca %s \n",aux2->nome,nome2);
...
void main ()
{
   ...
   int opcao;
   char thalys[100];
   strcpy(thalys,"Thalys");
   char marques[100];
   strcpy(marques,"Marques");
   
   struct user* home;
   home=NULL;
   
   home=adicionarestatico(home,thalys);
   home=adicionarestatico(home,marques);
